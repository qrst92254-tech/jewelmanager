...
router.get('/', (req, res) => {
  ...
  try {
    res.json(toObjects(db.exec('SELECT * FROM quotations ORDER BY created_at DESC')));
...
router.get('/:id', (req, res) => {
  const db = getDatabase();
  try {
    const rows = toObjects(db.exec('SELECT * FROM quotations WHERE id=?', [req.params.id]));
...
router.post('/', (req, res) => {
  ...
  try {
    ...
    const cnt = db.exec('SELECT COUNT(*) as c FROM quotations')[0]?.values[0][0] || 0;
    const quotation_number = `QUO-${yr}-${String(Number(cnt)+1).padStart(4,'0')}`;
    db.run(`INSERT INTO quotations 
      (quotation_number,customer_name,customer_phone,gold_rate_used,silver_rate_used,valid_until,subtotal,gst_amount,grand_total,notes)
      VALUES (?,?,?,?,?,?,?,?,?,?)`,
      [quotation_number, customer_name||null, customer_phone||null, gold_rate_used||null,
...
router.put('/:id', (req, res) => {
  ...
  try {
    db.run('UPDATE quotations SET status=?,converted_to_sale_id=? WHERE id=?',
      [status, converted_to_sale_id||null, req.params.id]);
    saveDatabase();
