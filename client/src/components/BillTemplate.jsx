...
const BillTemplate = forwardRef(({ sale }, ref) => {
    if (!sale) return null;

    const renderPrice = (value) => '₹' + (value || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

    return (
        <div ref={ref} style={{ padding: '2.5rem', background: 'white', color: '#1A1A1A', fontFamily: "'DM Sans', sans-serif", maxWidth: '800px', margin: '0 auto' }}>
            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px double var(--gold, #B8960C)', paddingBottom: '1.5rem', marginBottom: '1.5rem' }}>
                <div>
                    <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '28px', fontWeight: 600, color: '#B8960C', margin: '0 0 0.25rem 0' }}>
                        💎 JewelManager Pro
                    </h1>
                    <p style={{ fontSize: '0.85rem', color: '#666', margin: '0.15rem 0' }}>123 Gemstone Lane, Jewel City, 110022</p>
                    <p style={{ fontSize: '0.85rem', color: '#666', margin: '0.15rem 0' }}>Phone: +91 98765 43210</p>
                    <p style={{ fontSize: '0.85rem', color: '#666', margin: '0.15rem 0', fontWeight: 600 }}>GSTIN: 07ABCDE1234F1Z5</p>
                </div>
            ...
            <div style={{ borderTop: '1px solid #E5E1D8', paddingTop: '1.5rem', textAlign: 'center' }}>
                <p style={{ fontSize: '0.85rem', color: '#666', marginBottom: '0.25rem' }}>Thank you for your business!</p>
